...
var6="${RED}X${RESET}"
timeloc=/etc/kismet/timechk
kiss_state="`sudo pidof kismet`"
...
if [ "`gpspipe -w -n 10 | grep TPV | tail --lines=1 | cut -d'"' -f14 | cut --bytes=1-10,12-16 | sed -e "s/^\(.\{10\}\)/\1 /"`" == "`TZ=":UTC" date "+%Y-%m-%d %R"`" ]; then
      ...
      python /etc/kismet/correct_date.py
      var6="${GREEN}OK${RESET}"
fi
}
...
`echo -e "${RED} /____| \_/ |_|_| |_|_|\_\__,_|_| \_\  ${RESET}"`
 ${UND} _____      _____ ${RESETU}   ${UND} _        _ ${RESETU}
${RED}//     \\${RESET}${UND}MENU${RESETU}${RED}/     \\\\${RESET} ${RED}// \\${RESET}${UND}STATUS${RESETU}${RED}/ \\\\${RESET}
...
${RED}\\${RESET}${UND}\\${RESETU}${UND}________________/${RESETU}${RED}/${RESET} ${RB} ${RESET1} ${RED}AP${RESET}    ${RED}>${RESET} `printf "%-12s ${RB} ${RESET1}\n" "$var7"`
 ${UND} __________________${RESETU}${RED}//${RESET}    ${UND} ______/${RESETU}${RED}/${RESET}
${RED}// ${RESET}                      ${RED}//${RESET}
${RB} ${RESET1}`printf "%-31s  ${RED}//${RESET}\n" "$var8"`
${RB} ${RESET1}${RED}Total APs >${RESET} `printf "%-20s${RED}//${RESET}\n" "$kk"` $var5
${RED}\\${RESET}${UND}\\${RESETU}${UND}____________________/${RESETU}${RED}/${RESET}
EOF
...
/usr/bin/kismet_server --daemonize > /dev/null 2>&1
var5="${RED}ARMED${RESET}"
sleep 2
else
